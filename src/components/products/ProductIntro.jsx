...
const ProductIntro = ({ product }) => {
  const { id, title, brand, price, size, dimension, slug, thumbnail, images, description } = product;
          ...
          <FlexBox alignItems="center" mb={2}>
            <Box component="ul" sx={{ listStyle: "disc"}}>
                {Array.isArray(description) ? description.map((line, index) => (
                  ["FEATURE", "FEATURES", "SPECIFICATION", "SPECIFICATIONS", "DETAIL", "DETAILS"].includes(line.trim().toUpperCase()) ? (
                    <Box key={index} sx={{marginLeft: "-17px", marginTop: "15px", fontWeight: 400}}>
                      {line}
                    </Box> // Render without bullet for the first line if it matches
                  ) : (
