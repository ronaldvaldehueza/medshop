...
const StyledMartCard = styled(MartCard)(({ theme }) => ({
  margin: "auto",
  height: "100%",
  display: "flex",
  overflow: "hidden",
...
const ProductCard14a = (props) => {
  ...
  return (
    <StyledMartCard hoverEffect={hoverEffect}>
      <ImageWrapper style={{ height: "282px"}}> {/* STAR CUSTOM */}
        {/* {off !== 0 && (
          <StyledChip color="primary" size="small" label={`${off}% off`} />
        )} */}

        <ImageBox style={{ height: "282px"}}> {/* STAR CUSTOM */}
          <Link href={`/product/${slug}`}>
              <LazyImage
                alt={title || 'default title'} // Empty alt or src attributes can confuse the React hydration process.
                src={imgUrl || '/assets/images/amslogo.png'}
                width={190}
                height={190}
                layout="responsive"
                objectFit="contain"
        ...
        <Box flex="1 1 0" minWidth="0px" mr={1}>
          ...
          {!hideRating && (
            <Box display="flex" alignItems="center">
              <MartRating value={rating || 0} color="warn" readOnly />{" "}
              {/* <Span
                sx={{
